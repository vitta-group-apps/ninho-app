...
import {
  groupLogsByTimeOfDay, TIME_OF_DAY_EMOJI,
  analyzeDayPatterns, type TimeOfDay,
...
function DailyStats({ logs }: { logs: RoutineLog[] }) {
  ...
  return (
    <div className="flex gap-2.5 mb-4">
      <SummaryMetricCard emoji="🤱" label="Mamadas"
        value={feeds > 0 ? `${feeds}` : '—'}
        sub={insights.avgFeedIntervalMin ? `~${insights.avgFeedIntervalMin}min entre mamadas` : undefined}
        accentColor={FEED_COLOR} empty={feeds === 0}
      />
      <SummaryMetricCard emoji="🧷" label="Fraldas"
        value={diapers > 0 ? `${diapers}` : '—'}
        accentColor={DIAPER_COLOR} empty={diapers === 0}
      />
      <SummaryMetricCard emoji="😴" label="Sono"
        value={sleepLabel}
        sub={insights.hasLongSleep ? 'Inclui sono longo' : undefined}
        accentColor={SLEEP_COLOR} empty={sleepSec === 0}
      />
    </div>
  );
}

function QuickNoteSheet({ open, onClose, childId }: { open: boolean; onClose: () => void; childId: string }) {
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = await supabase.from('routine_logs').insert({
        child_id: childId, author_id: user.id, type: 'note',
      ...
      >
        ...
        <div className="px-5 pb-8 space-y-4">
          <div className="flex items-center gap-2">
            <PencilSquareIcon className="w-5 h-5" style={{ color: TXT_MUTED }} />
            <p className="text-[16px] font-bold font-quicksand" style={{ color: TXT }}>Nota rápida</p>
          </div>
          <textarea
...
function FAB({ onBreastfeed, onBottle, onSleep, onDiaper, onNote }: {
  onBreastfeed: () => void; onBottle: () => void;
  onSleep: () => void; onDiaper: () => void; onNote: () => void;
}) {
      ...
      <div className="fixed bottom-24 right-4 z-50 flex flex-col items-end gap-2.5">
        <AnimatePresence>
          {open && actions.map((a, i) => (
            ...
            >
              <span className="text-[12px] font-bold px-3 py-1.5 rounded-full font-nunito"
                style={{
                  color: a.color, backgroundColor: CARD_BG,
              ...
              <button onClick={() => { a.onClick(); setOpen(false); }}
                ...
                style={{ backgroundColor: a.color, boxShadow: '0 3px 10px rgba(0,0,0,0.15)' }}>
        ...
        <button onClick={() => setOpen(v => !v)}
          ...
          style={{ backgroundColor: MAUVE, boxShadow: '0 4px 16px rgba(0,0,0,0.18)' }}>
...
function GroupedSection({ group, logs, onTap }: { group: TimeOfDay; logs: RoutineLog[]; onTap: (log: RoutineLog) => void }) {
  ...
  return (
    <div className="mb-4">
      <button onClick={() => setCollapsed(v => !v)}
        ...
        <p className="text-[11px] font-bold uppercase tracking-[0.08em] font-nunito flex-1" style={{ color: TXT_MUTED }}>
          {group} · {logs.length} evento{logs.length !== 1 ? 's' : ''}
        </p>
        <span className="text-[10px] font-nunito opacity-60" style={{ color: TXT_MUTED }}>
...
export default function RotinaPage() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .gte('start_time', from.toISOString()).order('start_time', { ascending: false });
      if (error) throw error;
      setAllLogs(data ?? []);
    } catch { /* silent */ }
  ...
  function handleNoteClose() { setNoteSheetOpen(false); loadLogs(); }

  const filteredLogs = allLogs.filter(log => {
    if (typeFilter !== 'all' && log.type !== typeFilter) return false;
    if (todFilter) {
      const h = new Date(log.start_time).getHours();
      const tod: Record<string, [number, number]> = { Manhã: [5, 12], Tarde: [12, 18], Noite: [18, 22], Madrugada: [22, 29] };
  ...
  return (
    <div className="min-h-screen pb-28" style={{ backgroundColor: PAGE_BG }}>
      ...
      >
        <h1 className="text-[22px] font-bold font-quicksand" style={{ color: 'white' }}>Rotina</h1>
        ...
        {lastFeed && (
          <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full"
            ...
            <p className="text-[12px] font-semibold font-nunito" style={{ color: 'rgba(255,255,255,0.9)' }}>
      ...
      <div className="px-4 pt-3 space-y-2">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl"
          ...
          <MagnifyingGlassIcon className="w-4 h-4 flex-shrink-0" style={{ color: TXT_MUTED }} strokeWidth={2} />
          ...
          {showFilters && (
            ...
            >
              ...
              <div className="flex items-center justify-between px-1">
                <p className="text-[12px] font-semibold font-nunito" style={{ color: TXT_MUTED }}>
                  Agrupar por período do dia
                </p>
                <button onClick={() => setGroupByTod(v => !v)}
      ...
      <div className="px-4 pt-3">
        {childLoading ? (
          ...
          <div className="flex flex-col items-center justify-center pt-16 text-center">
            ...
            <p className="text-[16px] font-bold font-quicksand" style={{ color: TXT }}>Nenhuma criança ativa</p>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            {hasActiveFilters && (
              <div className="flex items-center justify-between mb-3">
                <p className="text-[11px] font-bold uppercase tracking-[0.08em] font-nunito" style={{ color: TXT_MUTED }}>
                  {filteredLogs.length} resultado{filteredLogs.length !== 1 ? 's' : ''}
                </p>
                <button onClick={() => { setSearch(''); setTypeFilter('all'); setTodFilter(''); setPeriod('today'); }}
            ...
            {logsLoading ? (
              ...
              <div className="rounded-2xl px-5 py-10 text-center"
                ...
                <p className="text-[15px] font-bold font-quicksand" style={{ color: TXT }}>
                  {search ? 'Nenhum resultado encontrado' : hasActiveFilters ? 'Nenhum evento com esses filtros' : 'Nenhum evento registrado'}
                </p>
                <p className="text-[13px] mt-1.5 font-nunito leading-snug max-w-[240px] mx-auto" style={{ color: TXT_MUTED }}>
                  {search ? `Sem resultados para "${search}".` : hasActiveFilters ? 'Ajuste os filtros para ver mais eventos.' : 'Toque no + para registrar o primeiro evento do dia.'}
                </p>
                {hasActiveFilters && (
                  <button onClick={() => { setSearch(''); setTypeFilter('all'); setTodFilter(''); setPeriod('today'); }}
              ...
              <div>
                {groupLogsByTimeOfDay(filteredLogs).map(({ group, logs: groupLogs }) => (
                  <GroupedSection key={group} group={group} logs={groupLogs} onTap={() => {}} />
                ))}
              </div>
            ) : (
              <div>
                {filteredLogs.map((log, idx) => (
                  <EventCard key={log.id} log={log} isLast={idx === filteredLogs.length - 1} onTap={() => {}} />
                ))}
              </div>
            )}
          </motion.div>
      ...
      <AnimatePresence>
        {noteSheetOpen && activeChild && (
          <QuickNoteSheet open={noteSheetOpen} onClose={handleNoteClose} childId={activeChild.id} />
        )}
      </AnimatePresence>
    </div>
  );
