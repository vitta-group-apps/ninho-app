 ...
 *
 * Same EventCard as Home. Same DS components.
 * No improvised per-screen patterns.
 */
...
function DailyStats({ logs }: { logs: RoutineLog[] }) {
  ...
  return (
    <div className="flex gap-2 mb-5">
      <SummaryMetricCard emoji="🤱" label="Mamadas"  value={String(feeds)}   accentColor={FEED_COLOR} />
      <SummaryMetricCard emoji="🧷" label="Fraldas"  value={String(diapers)} accentColor={DIAPER_COLOR} />
      <SummaryMetricCard emoji="😴" label="Sono"     value={sleepLabel}      accentColor={SLEEP_COLOR} />
    </div>
  );
}
      ...
      <div className="fixed bottom-24 right-4 z-50 flex flex-col items-end gap-2">
        <AnimatePresence>
          {open && actions.map((a, i) => (
            ...
            >
              <span
                className="text-xs font-bold px-3 py-1.5 rounded-full font-nunito bg-card"
                style={{ color: a.color, boxShadow: '0 2px 8px rgba(0,0,0,0.12)' }}
              >
                {a.label}
              </span>
              <button
                onClick={() => { a.onClick(); setOpen(false); }}
                className="w-12 h-12 rounded-full flex items-center justify-center shadow-md transition-all active:scale-90 text-white"
...
export default function RotinaPage() {
  ...
  return (
    <div className="min-h-screen pb-28 bg-background">
      ...
      >
        <h1 className="text-2xl font-bold text-white font-quicksand">Rotina</h1>
        <p className="text-sm text-white/70 mt-0.5 font-nunito">
        ...
        {lastFeed && (
          <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/15">
            ...
            <p className="text-xs text-white/90 font-semibold font-nunito">
      ...
      <div className="px-4 pt-4">
        {childLoading ? (
          ...
          <div className="flex flex-col items-center justify-center pt-16 text-center">
            ...
            <p className="text-base font-bold font-quicksand text-foreground">Nenhuma criança ativa</p>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            {logsLoading ? (
              ...
              <div className="rounded-2xl px-5 py-10 text-center bg-card border border-border">
                ...
                <p className="text-xs mt-1 text-muted-foreground font-nunito">
