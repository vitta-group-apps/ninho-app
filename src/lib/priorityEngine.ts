...
export function runPriorityEngine(params: {
  ...
  const dueUnconfirmed =
    appliedVaccineCount >= 0
  ...
  if (
    ...
    feedThresholdH &&
    ageMonths < 12 &&
    !allItems.find(i => i.id === 'no-feeds-today')
  ) {
  ...
  const growthSuggestDays = getGrowthSuggestDays(bucket);
  void growthSuggestDays;
  ...
  void consultThresholdDays;

  if (consultationCount === 0 && ageMonths >= 0) {
    allItems.push({
      id: 'no-consultation',
      level: 'suggested_next',
      emoji: '🩺',
  ...
  if (ongoingSleep) {
    ...
    mainItemId = 'ongoing-sleep';
  } else if (allItems.length > 0) {
    const healthOrMissing = allItems.find(
      i => i.level === 'health_risk' || i.level === 'missing_care'
    );
